...
%%% @doc
%%% This module contains erlcash API
%%% @end
%%%-------------------------------------------------------------------
-module(erlcash).
-author("Madalin Grigore-Enescu").
...
-spec create(PoolConfig) -> {ok, Pid} | {error, Reason} when
    PoolConfig	:: erlcash_config(),
    Pid        	:: pid(),
    Reason     	:: term().
create(PoolConfig) ->

    %% Get main supervisor reference
    MainSupRef = application:get_env(erlcash, sup_ref, ?erlcash_DEFAULT_SUP_REF),

    %% Build pool supervisor child specifications
    ChildSpec   = erlcash_utils:pool_config_to_child_specs(PoolConfig),
...
delete(PoolName) ->
    ...
    IdSupPool   = erlcash_utils:id_sup_pool(PoolName),
...
%% @doc List all pools
-spec list() -> PoolsList when PoolsList :: [erlcash_name()].
list() ->
...
list([[{Id, _Child, supervisor, _Modules}]|T], Acum) ->
    ...
    case erlcash_utils:id_sup_pool_to_pool_name(Id) of
...
take(PoolName, Wait, Timeout) ->
    ...
    try
        ...
        Type:Exception ->
            ...
            %% Notice that the warning report is generated only when erlcash_debug preprocessor directive is enabled
            ...
            ?erlcash_LOG_WARNING_REPORT(take_exception, [{pool, PoolName}, {timeout, Timeout}, {exception, {Type, Exception}}]),
...
grouping_add_pool(GroupName, PoolName) when erlang:is_atom(GroupName) ->
    ...
    GroupingServerRef = application:get_env(erlcash, srv_grouping_ref, ?erlcash_DEFAULT_SRV_GROUPING_REF),
...
grouping_list_pools(GroupName) ->
    ...
    %% the grouping server.
    ets:match(?erlcash_ETS_GROUPING_POOLS_BY_GROUP, {GroupName, '$1'}).
