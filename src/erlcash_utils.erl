...
id_sup_pool(PoolName) -> erlang:list_to_atom("erlcash_sup_pool_" ++ erlang:atom_to_list(PoolName)).

%% @doc Returns the workers supervisor id of the specified pool
id_sup_workers(PoolName) -> erlang:list_to_atom("erlcash_sup_workers_" ++ erlang:atom_to_list(PoolName)).

%% @doc Returns the workers manager server id of the specified pool
id_srv_workers_manager(PoolName) -> erlang:list_to_atom("erlcash_srv_workers_manager_" ++ erlang:atom_to_list(PoolName)).
...
config_get_value(Key, PoolConfig, Default) when erlang:is_list(PoolConfig) ->
    proplists:get_value(Key, PoolConfig, application:get_env(erlcash, Key, Default)).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% pool config
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% @doc Convert pool options to erlcash_sup supervisor child specs
...
pool_config_to_child_specs(PoolConfig = #erlcash_config{
    name                = Name,
    sup_pool_shutdown   = Shutdown
}) ->

    %% Build pool supervisor id
    IdSupPool = erlcash_utils:id_sup_pool(Name),

    %% We use legacy specs for now to remain compatible with older erlang versions we still want to support
    {IdSupPool, {erlcash_sup_pool, start_link, [PoolConfig]}, transient, Shutdown, supervisor, [erlcash_sup_pool]}.

%% @doc Create a erlcash_options record from the specified proplist
-spec pool_config_from_proplist(PoolConfig) -> erlcash_config() when
    PoolConfig :: [{atom(), term()}].
pool_config_from_proplist(PoolConfig) when erlang:is_list(PoolConfig) ->

    %% Extract config parameters from proplist and creates the erlcash config record
    #erlcash_config{
